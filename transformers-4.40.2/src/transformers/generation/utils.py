...
class GenerationMixin:
    ...
    def generate(
        self,
        inputs: Optional[torch.Tensor] = None,
        inputs_ids_student: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if generation_config_student.pad_token_id is None and generation_config_student.eos_token_id is not None:
            if model_kwargs_student.get("attention_mask", None) is None:
                logger.warning(
                    "The attention mask and the pad token id were not set. As a consequence, you may observe "
                    "unexpected behavior. Please pass your input's `attention_mask` to obtain reliable results."
                )
            eos_token_id = generation_config_student.eos_token_id
            if isinstance(eos_token_id, list):
                eos_token_id = eos_token_id[0]
            logger.warning(f"Setting `pad_token_id` to `eos_token_id`:{eos_token_id} for open-end generation.")
            generation_config_student.pad_token_id = eos_token_id
        ...
        inputs_tensor_student, model_input_name_student, model_kwargs_student = self._prepare_model_inputs(
            inputs_ids_student, generation_config_student.bos_token_id, model_kwargs_student
        )
        ...
        model_kwargs_student["output_attentions"] = generation_config_student.output_attentions
        model_kwargs_student["output_hidden_states"] = generation_config_student.output_hidden_states
        # decoder-only models with inputs_embeds forwarding must use caching (otherwise we can't detect whether we are
        # generating the first new token or not, and we only want to use the embeddings for the first new token)
        if not self.config.is_encoder_decoder and model_input_name_student == "inputs_embeds":
            model_kwargs_student["use_cache"] = True
        else:
            model_kwargs_student["use_cache"] = generation_config_student.use_cache
        ...
        if model_kwargs_student.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            model_kwargs_student["attention_mask"] = self._prepare_attention_mask_for_generation(
                inputs_tensor_student, generation_config_student.pad_token_id, generation_config_student.eos_token_id
            )
        ...
        if self.config.is_encoder_decoder:
            ...
            input_ids_student = inputs_tensor_student if model_input_name_student == "input_ids" else model_kwargs_student.pop("input_ids")
        ...
        input_ids_seq_length = input_ids_length#.deepcopy()
        input_ids_seq_length_student = input_ids_length_student#.deepcopy()
        ...
        logits_processor_student = self._get_logits_processor(
            generation_config=generation_config_student,
            input_ids_seq_length=input_ids_seq_length_student,
        ...
        if is_greedy_gen_mode and deck_decoding:
        ...
        if is_greedy_gen_mode and contrastive_decoding:
        ...
        elif is_sample_gen_mode and deck_decoding:
            # 11. prepare logits warper
            logits_warper = self._get_logits_warper(generation_config)
            logits_warper_student = self._get_logits_warper(generation_config_student)
            ...
            input_ids_student, model_kwargs_student = self._expand_inputs_for_generation(
            ...
            # 13. run sample
            return self.deck_sample(
                input_ids,
                input_ids_student,
                logits_processor=logits_processor,
                logits_processor_student=logits_processor_student,
    ...
    def relative_top_filter2(self, scores: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 2) -> torch.FloatTensor:
        sorted_logits, sorted_indices = torch.sort(scores, descending=True)
        min_thresh = sorted_logits[..., min_tokens_to_keep-1]
        probs_max = torch.max(sorted_logits, dim=-1).values
        probs_thresh = probs_max + np.log(relative_top)
        probs_thresh = torch.min(min_thresh, probs_thresh)
        probs_thresh = probs_thresh.unsqueeze(-1)
        scores[scores < probs_thresh] = filter_value
        return scores
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        while True: #self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            # prepare model inputs
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
    ...
    def deck_sample(
        ...
        relative_top: float = 0.1,
        ref_mask: torch.LongTensor = None,
        logits_processor: Optional[LogitsProcessorList] = None,
        logits_processor_student: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        logits_warper: Optional[LogitsProcessorList] = None,
        ...
        **model_kwargs,
        ...
        logits_processor = logits_processor if logits_processor is not None else LogitsProcessorList()
        logits_processor_student = logits_processor_student if logits_processor is not None else LogitsProcessorList()
        stopping_criteria = stopping_criteria if stopping_criteria is not None else StoppingCriteriaList()
        if max_length is not None:
            warnings.warn(
                "`max_length` is deprecated in this function, use"
            ...
            stopping_criteria = validate_stopping_criteria(stopping_criteria, max_length)
        logits_warper = logits_warper if logits_warper is not None else LogitsProcessorList()
        logits_warper_student = logits_warper_student if logits_warper_student is not None else LogitsProcessorList()
        pad_token_id = pad_token_id if pad_token_id is not None else self.generation_config.pad_token_id
        eos_token_id = eos_token_id if eos_token_id is not None else self.generation_config.eos_token_id
        if isinstance(eos_token_id, int):
            eos_token_id = [eos_token_id]
        ...
        # keep track of which sequences are already finished
        unfinished_sequences = torch.ones(input_ids.shape[0], dtype=torch.long, device=input_ids.device)
        unfinished_sequences_student = torch.ones(input_ids_student.shape[0], dtype=torch.long, device=input_ids_student.device)
        ...
        while True:
            ...
            logits_student = outputs_student.logits[:, -1, :]
            ...
            st_coef = 0.5
            # ini_logits = copy.deepcopy(logits)
            # ini_sdu = copy.deepcopy(logits_student)
            # if relative_top > 0.0:
            #     # st_coef = 10
            #     logits = self.relative_top_filter2(logits, relative_top, min_tokens_to_keep=3)
            #     # logits_student = logits_student.log_softmax(dim=-1)
            #     mask = logits[0] < -1e3
            ...
            if ref_mask is not None:
                ...
                # print('After:', logits)
            if relative_top > 0.0:
                logits = self.relative_top_filter(logits, relative_top, min_tokens_to_keep=5)
                logits_student = logits_student.log_softmax(dim=-1)
                mask = logits[0] < -1e3
                logits_student[0][mask] = -1e3
            
            # sorted_logits, sorted_indices = torch.sort(logits, descending=True, dim=-1)
            # sorted_slogits, sorted_sindices = torch.sort(logits_student, descending=True, dim=-1)
            # ini_sorted_logits, ini_sorted_indices = torch.sort(ini_logits, descending=True, dim=-1)
            ...
            # sample
            probs = nn.functional.softmax(next_token_scores, dim=-1)
            # sorted_fprobs, sorted_findices = torch.sort(probs, descending=True, dim=-1)
